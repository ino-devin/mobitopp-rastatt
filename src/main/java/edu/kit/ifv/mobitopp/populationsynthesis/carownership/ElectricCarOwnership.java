...
public class ElectricCarOwnership implements ProbabilityForElectricCarOwnershipModel {
	...
	final Double STADT_BEV = null;
	final Double KLEINSTADT_BEV = null;
	final Double STADTRAUM_BEV = null;
	final Double LANDRAUM_BEV = null;
	...
	final Double STADT_EREV = null;
	final Double KLEINSTADT_EREV = null;
	final Double STADTRAUM_EREV = null;
	final Double LANDRAUM_EREV = null;

	public ElectricCarOwnership(ImpedanceIfc impedance, String configFile) {
		this.impedance = impedance;
		new ParameterFormularParser().parseConfig(new File(configFile), this);
	}
	...
	private double bevUtility(PersonBuilder person) {
		...
		return CONST_BEV
				...
				+ STADT_BEV * regionTypeStadt(person)
				+ KLEINSTADT_BEV * regionTypeKleinstadt(person)
				+ STADTRAUM_BEV * regionTypeStadtraum(person)
				+ LANDRAUM_BEV * regionTypeLandraum(person);
	}
	...
	private double erevUtility(PersonBuilder person) {
		...
		return CONST_EREV
				...
				+ STADT_EREV * regionTypeStadt(person)
				+ KLEINSTADT_EREV * regionTypeKleinstadt(person)
				+ STADTRAUM_EREV * regionTypeStadtraum(person)
				+ LANDRAUM_EREV * regionTypeLandraum(person);
	}
	...
	private static int regionTypeStadt(PersonBuilder person) {
		int regionType = regionTypeOf(person);
		return regionType == 121 || regionType == 211 || regionType == 111 || regionType == 112 ? 1 : 0;
	}

	private static int regionTypeKleinstadt(PersonBuilder person) {
		int regionType = regionTypeOf(person);
		return regionType == 113 || regionType == 123 || regionType == 213 || regionType == 223
				|| regionType == 221 ? 1 : 0;
	}

	private static int regionTypeStadtraum(PersonBuilder person) {
		int regionType = regionTypeOf(person);
		return regionType == 114 || regionType == 124 || regionType == 214 ? 1 : 0;
	}

	private static int regionTypeLandraum(PersonBuilder person) {
		int regionType = regionTypeOf(person);
		return regionType == 115 || regionType == 125 || regionType == 215 || regionType == 224
				|| regionType == 225 ? 1 : 0;
	}

	private static int regionTypeOf(PersonBuilder person) {
		return person.homeZone().getRegionType().code();
	}
}
...
public class ParallelPersonStep extends PersonBasedStep {
	
	private static ExecutorService executor;
	private static List<Callable<Void>> tasks = new ArrayList<>();
	
	public ParallelPersonStep(Consumer<PersonBuilder> consumer) {
		super(wrapConsumer(consumer));
	}
	
	public ParallelPersonStep(Consumer<PersonBuilder> consumer, int threads) {
		this(consumer);

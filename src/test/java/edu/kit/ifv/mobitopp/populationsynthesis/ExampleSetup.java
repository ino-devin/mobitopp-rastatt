...
public abstract class ExampleSetup {
	...
	public static final int hhType = 1;
	...
	private static final EconomicalStatus economicalStatus = EconomicalStatus.veryLow;
	...
	private static Collection<PrivateCarForSetup> cars(HouseholdId household, PersonBuilder personForDemand,
			Zone zone) {
	...
	public static PrivateCarForSetup conventionalCar(HouseholdForSetup household, PersonBuilder mainUser,
			PersonBuilder personalUser, Zone zone) {
		Car car = conventionalCar(zone);
	...
	public static ConventionalCar conventionalCar(Zone zone) {
		...
		return new ConventionalCar(carId, position, segment, capacity, initialMileage, fuelLevel, maxRange);
	}

	public static BatteryElectricCar bevCar(Zone zone) {
		CarPosition position = new CarPosition(zone, location);
		return new BatteryElectricCar(carId, position, segment, capacity, initialMileage, batteryLevel, electricRange,
				batteryCapacity, minimumChargingLevel);
	}

	public static ExtendedRangeElectricCar erevCar(Zone zone) {
		CarPosition position = new CarPosition(zone, location);
		return new ExtendedRangeElectricCar(carId, position, segment, capacity, initialMileage, batteryLevel,
				conventionalFuelLevel, electricRange, conventionalRange, fullPowerRange, batteryCapacity,
	...
	public static HouseholdForSetup household(Zone zone, int householdOid, int numberOfCars) {
		...
		return new DefaultHouseholdForSetup(id, nominalSize, domcode, hhType, zone, location, numberOfMinors,
	...
	public static PersonBuilder personOf(HouseholdForSetup household, int personNumber, Zone zone,
		...
		PersonBuilder person = new DefaultPersonForSetup(id, household, age, employment, gender, graduation, income,
				ModeChoicePreferences.NOPREFERENCES).setHasBike(hasBike).setHasAccessToCar(hasAccessToCar)
						.setHasPersonalCar(hasPersonalCar).setHasCommuterTicket(hasCommuterTicket)
						.setHasDrivingLicense(hasLicense);
